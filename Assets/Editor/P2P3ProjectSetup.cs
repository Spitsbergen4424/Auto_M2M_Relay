...
public static class P2P3ProjectSetup
{
    ...
    public static void Configure()
    {
        ...
        // The policy already uses an LSTM, so observation stacking only duplicates history.
    ...
    {
        if (GameObject.Find("GFSX_Robot") == null && File.Exists(ScenePath))
        {
        ...
        Require(robotCamera != null && robotCamera.enabled, "Robot camera must be enabled.");
        ...
        Require(behavior.BrainParameters.VectorObservationSize == 15 &&
                behavior.BrainParameters.NumStackedVectorObservations == 1, "Observation layout must be 15 x 1.");
        Require(behavior.BrainParameters.ActionSpec.NumContinuousActions == 3 &&
                ...
                behavior.BrainParameters.ActionSpec.BranchSizes[0] == 2, "Action layout must be 3 continuous + [2].");
