...
public static class GfsxRealRobotSetup
{
    ...
    public static void Configure()
    {
        ...
        if (robotPrefab == null)
        {
            throw new InvalidOperationException($"Robot prefab was not found at {RobotPrefabPath}.");
        }
        ...
        RealRobotSensors realSensors = GetOrAdd<RealRobotSensors>(robot);
        GfsxRealRobotBridge bridge = GetOrAdd<GfsxRealRobotBridge>(robot);
        RobotBrain brain = GetRequiredComponent<RobotBrain>(robot);
        SimulatedYoloCamera simulatedCamera = robot.GetComponentInChildren<SimulatedYoloCamera>(true);
        TrackController trackController = GetRequiredComponent<TrackController>(robot);
        GripperController gripper = GetRequiredComponent<GripperController>(robot);
        Transform robotCameraTransform = FindChildRecursive(robot.transform, "RobotCamera");
        if (robotCameraTransform == null)
        {
            throw new InvalidOperationException("RobotCamera child is missing on the robot prefab.");
        }
    ...
    public static void Validate()
    {
        ...
        {
            throw new InvalidOperationException("RealRobotScene is missing. Run Configure Real Robot Scene first.");
        }

        Scene scene = EditorSceneManager.OpenScene(ScenePath, OpenSceneMode.Single);
        RobotBrain[] brains = UnityEngine.Object.FindObjectsByType<RobotBrain>(FindObjectsInactive.Include,
            FindObjectsSortMode.None);
        RealYoloCamera[] realCameras = UnityEngine.Object.FindObjectsByType<RealYoloCamera>(FindObjectsInactive.Include,
            FindObjectsSortMode.None);
        GfsxRealRobotBridge[] realBridges = UnityEngine.Object.FindObjectsByType<GfsxRealRobotBridge>(
            FindObjectsInactive.Include, FindObjectsSortMode.None);
        GfsxRosBridge[] simBridges = UnityEngine.Object.FindObjectsByType<GfsxRosBridge>(FindObjectsInactive.Include,
            FindObjectsSortMode.None);
        RealRobotSensors[] sensors = UnityEngine.Object.FindObjectsByType<RealRobotSensors>(
            FindObjectsInactive.Include, FindObjectsSortMode.None);

        Require(brains.Length == 1, $"Expected one RobotBrain, found {brains.Length}.");
        Require(realCameras.Length == 1, $"Expected one RealYoloCamera, found {realCameras.Length}.");
        Require(realBridges.Length == 1, $"Expected one GfsxRealRobotBridge, found {realBridges.Length}.");
        Require(sensors.Length == 1, $"Expected one RealRobotSensors, found {sensors.Length}.");
        ...
        Require(brain.SensorSource == realSensors, "RobotBrain must use RealRobotSensors as its sensor source.");
        Require(brain.PoseSource == bridge, "RobotBrain must use GfsxRealRobotBridge as its pose source.");
        Require(realCamera.ListenPort == 5005, $"RealYoloCamera must listen on 5005, found {realCamera.ListenPort}.");
        Require(bridge.RosIpAddress == "192.168.2.154", $"ROS IP must be 192.168.2.154, found {bridge.RosIpAddress}.");
        Require(bridge.RosPort == 10000, $"ROS port must be 10000, found {bridge.RosPort}.");
        Require(bridge.DryRun, "Dry run must be enabled by default.");
        ...
        SimulatedYoloCamera simulatedCamera = brain.GetComponentInChildren<SimulatedYoloCamera>(true);
        Require(simulatedCamera == null || !simulatedCamera.enabled,
        ...
        int bridgeCount = UnityEngine.Object.FindObjectsByType<GfsxRealRobotBridge>(FindObjectsInactive.Include,
            FindObjectsSortMode.None).Length;
        int simBridgeCount = UnityEngine.Object.FindObjectsByType<GfsxRosBridge>(FindObjectsInactive.Include,
            FindObjectsSortMode.None).Count(item => item.enabled);
        Require(bridgeCount == 1, $"Expected one physical bridge, found {bridgeCount}.");
        Require(simBridgeCount == 0, "Simulation GfsxRosBridge must be disabled or absent.");
        Require(UnityEngine.Object.FindObjectsByType<ROSConnection>(FindObjectsInactive.Include,
            FindObjectsSortMode.None).Length == 1, "RealRobotScene must contain one ROSConnection.");
    ...
    private static void SetupCameraRig(GameObject robot)
    {
        ...
        Transform cameraTransform = FindChildRecursive(robot.transform, "RobotCamera");
    ...
    {
        Camera overview = UnityEngine.Object.FindObjectsByType<Camera>(FindObjectsInactive.Exclude,
        ...
        overview.transform.SetParent(robot.parent, true);
        overview.transform.position = robot.TransformPoint(new Vector3(-3.2f, 1.8f, -1.8f));
    ...
    private static void RemoveTrainingOnlyChildren(Transform root)
    {
        foreach (Transform child in root.GetComponentsInChildren<Transform>(true).ToArray())
        {
            ...
            if (child.name.StartsWith("TrainingArena", StringComparison.OrdinalIgnoreCase) ||
                child.name.StartsWith("TargetBall", StringComparison.OrdinalIgnoreCase))
            {
    ...
    {
        foreach (GfsxRosBridge bridge in robot.GetComponentsInChildren<GfsxRosBridge>(true))
        {
            bridge.enabled = false;
        }
    }

    private static void DisableIfFound<T>(GameObject root) where T : Behaviour
    {
    ...
    private static T GetRequiredComponent<T>(GameObject target) where T : Component
    {
        ...
        if (component == null)
        {
            throw new InvalidOperationException($"{typeof(T).Name} is missing on {target.name}.");
        }
    ...
    private static void Require(bool condition, string message)
    {
        if (!condition)
        {
            throw new InvalidOperationException("Real robot scene validation failed: " + message);
        }
    }
}
