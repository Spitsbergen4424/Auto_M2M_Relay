...
public sealed class TrackController : MonoBehaviour
{
    ...
    private float baseCoastDeceleration;
    private float baseBrakingDeceleration;
    ...
    private void CaptureBaseline()
    {
        ...
        baseCoastDeceleration = coastDeceleration;
        baseBrakingDeceleration = brakingDeceleration;
        baselineCaptured = true;
    }

    // Domain randomization (training only): jitters drive dynamics around whatever was
    // calibrated at Awake (ConfigureScale's output or manual Inspector values), so a policy
    // learns to cope with worn gearboxes / uneven left-right tracks / a heavier battery
    // instead of overfitting to one exact motor response.
    public void RandomizeDynamics()
    {
        CaptureBaseline();
        moveSpeed = baseMoveSpeed * Random.Range(0.6f, 1.4f);
        turnSpeed = baseTurnSpeed * Random.Range(0.7f, 1.3f);
        acceleration = baseAcceleration * Random.Range(0.6f, 1.4f);
        coastDeceleration = baseCoastDeceleration * Random.Range(0.6f, 1.4f);
        brakingDeceleration = baseBrakingDeceleration * Random.Range(0.6f, 1.4f);
    }
    ...
    public void ResetDynamics()
    {
        ...
        coastDeceleration = baseCoastDeceleration;
        brakingDeceleration = baseBrakingDeceleration;
    }
